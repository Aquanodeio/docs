...
Manage your SSH keys from the [SSH Keys](https://console.aquanode.io/ssh-keys) page. You can add multiple keys and see which instances each key is attached to.

![SSH Keys management page showing saved keys and instance attachments](/virtual-machine/ssh-key-add.png)

1. Go to **SSH Keys** in your account settings and click **+ Add SSH Key**
...
4. Select the saved key when deploying a VM
...
2. **Optional**: Set deployment settings:
   ...
   - **Startup script**: Select a saved startup script to run on boot
...
Startup scripts let you automate workload initialization — install packages, clone repos, or configure your environment automatically every time a VM boots.

![Startup Scripts page showing empty state with Create First Script button](/virtual-machine/add-startup-script.png)

1. Go to [Startup Scripts](https://console.aquanode.io/workloads/startup-scripts) in your account settings
2. Click **+ New Script** (or **Create First Script** if you have none yet)
3. Write your shell script, give it a name, and save it
4. Select the script during VM deployment under **Startup script**
